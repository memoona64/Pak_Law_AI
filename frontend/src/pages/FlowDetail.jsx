...
export default function FlowDetail() {
  ...
  React.useEffect(() => {
    setLoading(true);
    setError('');
    fetch(`${API_URL}/api/flows/${encodeURIComponent(slug)}?lang=en`)
      .then(async (res) => {
        ...
        {flow && (
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-[16px] text-[#4A5540] max-w-[640px]">{flow.situation}</p>
            </div>
          </div>
        )}
      </div>
              ...
              <div className="bg-[#2A2F22] text-[#F7F6F0] p-6">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <div className="text-[14px] text-[#B9C2A0]/85 mt-1">{flow.situation}</div>
                  </div>
...
function StepDetail({ n, step, state, isOpen, onToggle }) {
  return (
    <div className={`rounded-lg border transition-colors ${isOpen ? 'bg-white border-[#6B7F5E]/40' : 'bg-[#F2F1E6]/60 border-[#DFE0CE] hover:border-[#6B7F5E]/40'}`}>
      <button onClick={onToggle} className="w-full flex items-center gap-3 px-4 py-3 text-left">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-serif text-[16px] leading-tight">{step.label}</div>
      ...
      {isOpen && (
        <div className="px-4 pb-4 pt-1">
          <div className="border-l-2 border-[#6B7F5E] pl-4 py-1">
            <p className="font-serif text-[16px] leading-[1.65] text-[#363B2C]">{step.body}</p>
          </div>
        </div>
      )}
    </div>
