...
//  * Protected route — req.user.id must be set by auth middleware.
//  */
...
//     // FastAPI does its own size/type/empty checks (413, 422) — analyzeDocument()
//     // surfaces those status codes rather than masking them as a generic error.
//     const analysis = await ragService.analyzeDocument(req.file.buffer, req.file.originalname, province);
...
//   } catch (error) {
//     if (error.status) {
//       // Errors surfaced from FastAPI (413 too large, 422 bad/empty document)
//       return res.status(error.status).json({ error: error.message });
//     }
//     next(error); // unexpected errors go to the central error handler
//   }
// };
...
//     if (!doc) {
//       // 404, not 403 — don't reveal whether a document exists for another user
